...
from helpers_zeroshot_dialect import *
...
def main():
    ...
    parser.add_argument('--type', default=None, type=str, help='Type of words.')
    parser.add_argument('--device', default=None, type=int, required=True, help='Selected CUDA device.')
    args = parser.parse_args()

    # Load data
    data = pd.read_json('../data/{}/zeroshot_{}.json'.format(args.data, args.type), lines=True)
    ...
    print('Word type: {}'.format(args.type))
    if args.mtl:
    ...
    if args.random:
        true = dataset.words
        preds = random.sample(dataset.words, len(dataset.words))
        print(accuracy_score(true, preds))
        with open('preds_zeroshot/{}_{}.p'.format(filename, args.type), 'wb') as f:
    ...
    for generation in range(1, 26):
        ...
        # Load data loaders
        collator = CollatorForZeroShot(tok, args.data)
        ...
        with torch.no_grad():
            for i, (batch_tensors, masked, words, pairs) in enumerate(data_loader):
                ...
                true.extend(words)
                for p, pair in zip(pred, pairs):
                    class_scores = [p[c].item() for c in pair]
                    preds.append(pair[np.argmax(class_scores)])
        ...
        with open('results_zeroshot/{}_{}.txt'.format(filename, args.type), 'a+') as f:
